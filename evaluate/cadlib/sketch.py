...
class Loop(SketchBase):
    ...
    def reorder(self):
        ...
        # 벡터의 마지막 차원에 0을 추가하여 3D로 변환
        end_vec_3d = np.append(end_vec, 0)
        start_vec_3d = np.append(start_vec, 0)

        # 3D 외적의 결과는 벡터이므로, z 성분([2])만 추출하여 비교
        if np.cross(end_vec_3d, start_vec_3d)[2] <= 0:
